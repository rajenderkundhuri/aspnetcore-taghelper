...
{
    [HtmlTargetElement("bs-modal-header")]
    public class ModalHeaderTagHelper : TagHelper
    {
        [HtmlAttributeName("class")]
        public string ModalHeaderClass { get; set; }=  string.Empty;

        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            output.Content.AppendHtml((await output.GetChildContentAsync()).GetContent());
            output.Attributes.SetAttribute("class", ("modal-header " + this.ModalHeaderClass));
