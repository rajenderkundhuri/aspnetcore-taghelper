...
{
    [HtmlTargetElement("bs-page-header")]
    public class PageHeaderTagHelper : TagHelper
    {
        [HtmlAttributeName("class")]
        public string PageHeaderClass { get; set; }=  string.Empty;

        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            output.Content.AppendHtml((await output.GetChildContentAsync()).GetContent());
            output.Attributes.SetAttribute("class", ("page-header " + this.PageHeaderClass));
