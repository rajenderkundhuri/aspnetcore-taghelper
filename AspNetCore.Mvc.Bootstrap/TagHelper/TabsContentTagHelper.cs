...
{
    [HtmlTargetElement("bs-tabs-content")]
    public class TabsContentTagHelper : TagHelper
    {
        [HtmlAttributeName("class")]
        public string TabsContentClass { get; set; }=  string.Empty;

        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            output.Content.AppendHtml((await output.GetChildContentAsync()).GetContent());
            output.Attributes.SetAttribute("class", ("tab-content " + this.TabsContentClass));
