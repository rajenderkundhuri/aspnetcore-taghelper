...
{
    [HtmlTargetElement("bs-carousel-items")]
    public class CarouselItemsTagHelper : TagHelper
    {
        [HtmlAttributeName("class")]
        public string CarouselItemsClass { get; set; }=  string.Empty;

        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            output.Content.AppendHtml((await output.GetChildContentAsync()).GetContent());
            output.Attributes.SetAttribute("class", ("carousel-inner " + this.CarouselItemsClass));
